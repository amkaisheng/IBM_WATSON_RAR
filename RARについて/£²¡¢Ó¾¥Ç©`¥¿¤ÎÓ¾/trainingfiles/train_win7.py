...
with open(RELEVANCE_FILE, 'rb') as csvfile:
    ...
    with open(TRAININGDATA, "a") as training_file:
        ...
        for row in question_relevance:
    	    question = row[0]
            enc_question = urllib.quote(question) 
    	    relevance = ','.join(row[1:])
            
            idx=idx+1
            print idx;
            
    	    curl_cmd = 'curl -k -s %s -u %s -d "q=%s&gt=%s&generateHeader=%s&rows=%s&returnRSInput=true&wt=json" "%s"' % (VERBOSE, CREDS, enc_question, relevance, add_header, ROWS, SOLRURL)
            if DEBUG:
                print (curl_cmd)
